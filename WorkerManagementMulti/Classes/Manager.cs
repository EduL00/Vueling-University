...
namespace Classes
{
    ...
    public class Manager
    {
        ...
        public void ListTeamMembers(MenuRole role, ITWorker manager)
        {
            ...
            for (int i = 0; i < Teams.Count; i++)
            {
                if ((role == MenuRole.Admin) || ((role == MenuRole.Manager) && (manager.Id == Teams[i].TeamManager.Id)))
                {
                    Console.WriteLine(Teams[i].Name);
                    Console.WriteLine("------------------------");
                    Teams[i].ListMembers();
        ...
        public void ListTaskAssignementsTeam(MenuRole role, ITWorker manager)
        {
            Console.Clear();
            Console.WriteLine("=======================");
            ...
            for (int i = 0; i < Teams.Count; i++)
            {
                if ((role == MenuRole.Admin) || ((role == MenuRole.Manager) && (Teams[i].TeamManager.Id == manager.Id)))
                {
                    Console.WriteLine($"{Teams[i].Name}");
                    Console.WriteLine("------------------------");
        ...
        public bool LookForWorker(string id, out ITWorker worker)
        {
            worker = null;
        ...
        public void ListWorkerTeamAssigments(ITWorker worker)
        {
            Team team = null;
            ...
            for (int j = 0; j < team.Technicians.Count; ++j)
            {
                for (int k = 0; k < AssignedTasks.Count; ++k)
                {
                    if (AssignedTasks[k].IdWorker == worker.Id)
                    {
                        AssignedTasks[k].ShowUnassignedTask();
                        Console.WriteLine("------------------------");
                    }
                }
            }
        ...
        public void SetYourselfToTask(ITWorker worker)
        {
            bool found_task;
            Task task = null;
